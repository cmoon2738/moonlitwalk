...
bool amw_vk_open_driver(void)
{
    ...
    module = amw_load_dll("vulkan-1.dll");

#elif defined(MW_PLATFORM_MACOSX) || defined(MW_PLATFORM_IOS)
    module = amw_load_dll("libvulkan.dylib");
    if (!module)
        module = amw_load_dll("libvulkan.1.dylib");
    if (!module)
        module = amw_load_dll("libMoltenVK.dylib");
    /* Add support for using Vulkan and MoltenVK in a Framework. App store rules for iOS
     * strictly enforce no .dylib's. If they aren't found it just falls through */
    if (!module)
        module = amw_load_dll("vulkan.framework/vulkan");
    if (!module)
        module = amw_load_dll("MoltenVK.framework/MoltenVK");
    ...
    if (!module && getenv("DYLD_FALLBACK_LIBRARY_PATH") == NULL)
        module = amw_load_dll("/usr/local/lib/libvulkan.dylib");

#else /* LINUX / BSD / ANDROID */
    module = amw_load_dll("libvulkan.so.1");
    if (!module)
        module = amw_load_dll("libvulkan.so");
#endif
    if (!module)
        return AMW_FALSE;
    vkGetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)amw_get_proc_address(module, "vkGetInstanceProcAddr");
