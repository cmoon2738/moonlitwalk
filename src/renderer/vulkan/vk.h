...
#define AMW_VK_MAX_FRAMES 3 /* 3 -> triple buffering */

typedef struct amw_vulkan amw_vulkan_t;
#define AMW_MOONY_BACKEND_VK_STATE amw_vulkan_t vk;
...
extern int32_t      AMW_CALL amw_vk_init(amw_moony_t *moony);
extern void         AMW_CALL amw_vk_terminate(amw_moony_t *moony);
extern void         AMW_CALL amw_vk_draw_frame(amw_moony_t *moony);

/** Vulkan driver version. */
extern uint32_t     AMW_CALL amw_vk_version(void);

/** Returns a string for a given VkResult. */
extern const char * AMW_CALL amw_vk_result(VkResult code);

#if defined(AMW_DEBUG) || defined(_DEBUG)
    #define AMW_VK_VERIFY(x) {                                                      \
        VkResult res = (x);                                                         \
        if (res != VK_SUCCESS) {                                                    \
            amw_log_error("VkResult verification failed: %s", amw_vk_result(res));  \
...
extern char *   AMW_CALL amw_vk_hadal_surface_extension(void);
extern bool     AMW_CALL amw_vk_physical_device_presentation_support(amw_window_t *window, VkPhysicalDevice device, uint32_t queue_family);
...
#else /* null the vulkan backend structures */
    #define AMW_MOONY_BACKEND_VK_STATE
#endif /* AMW_NATIVE_VULKAN */
AMW_C_DECL_END

#endif /* _amw_vulkan_h_ */
