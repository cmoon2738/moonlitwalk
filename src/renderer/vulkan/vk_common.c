...
static void instance_extensions(amw_arena_t *a, char **extensions, uint32_t *ext_count)
{
    ...
    if (extensions) {
        ...
        extensions[1] = amw_vk_hadal_surface_extension();
...
static amw_result_t create_instance(amw_moony_t *moony)
{
    ...
    if (layer_count == 0) {
        amw_log_error("\tValidation layers are not supported in this device, they will be disabled");
    ...
    amw_app_t *app_desc = a_moonlit_walk(NULL)->app;
    if (app_desc) {
    ...
    instance_extensions(&moony->temporary_arena, NULL, &extension_count);
    if (extension_count == 0) {
        amw_log_error("None of wanted Vulkan instance extensions are available.");
        return -1; // TODO
    }

    char **wanted_extensions = (char **)amw_arena_alloc(&moony->temporary_arena, sizeof(const char *) * extension_count);
    instance_extensions(&moony->temporary_arena, wanted_extensions, NULL);
    ...
    AMW_VK_VERIFY(vkCreateInstance(&create_info, NULL, &moony->vk.instance));
    amw_vk_load_instance_pointers(moony->vk.instance);

#if AMW_BUILD_VALIDATION_LAYERS
    if (validation_layers_enabled)
        amw_vk_create_validation_layers(moony->vk.instance);
#endif /* AMW_BUILD_VALIDATION_LAYERS */

    amw_log_verbose("Created a Vulkan instance...");
    amw_arena_reset(&moony->temporary_arena);
    return AMW_SUCCESS;
}
...
int32_t amw_vk_init(amw_moony_t *moony)
{
    int32_t res;
    ...
    res = create_instance(moony);
    if (res != AMW_SUCCESS) {
        amw_log_error("Could not create a Vulkan instance");
        amw_vk_terminate(moony);
        return res;
    }

    AMW_VK_VERIFY(amw_vk_surface_create(moony->vk.instance, moony->window, NULL, &moony->vk.surface));

    /* TODO move to shared code */
    amw_window_framebuffer_size(moony->window, &moony->fb_width, &moony->fb_height);
...
void amw_vk_terminate(amw_moony_t *moony)
{
    ...
    if (moony->vk.surface != VK_NULL_HANDLE)
        vkDestroySurfaceKHR(moony->vk.instance, moony->vk.surface, NULL);

#ifdef AMW_BUILD_VALIDATION_LAYERS
    /* save to call even if layers are disabled */
    amw_vk_destroy_validation_layers(moony->vk.instance);
#endif /* AMW_ENABLE_VALIDATION_LAYERS */

    if (moony->vk.instance != VK_NULL_HANDLE)
        vkDestroyInstance(moony->vk.instance, NULL);
    amw_vk_close_driver();

    /* TODO move to shared code */
    amw_arena_free(&moony->temporary_arena);
}

void amw_vk_draw_frame(amw_moony_t *moony)
{
    (void)moony;
}
