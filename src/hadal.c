...
void amw_hadal_set_clipboard(const char *string)
{
...
const char *amw_hadal_get_clipboard(void)
{
    if (!hadal.initialized) 
        return NULL;
...
uint32_t amw_window_flags(amw_window_t *window)
{
    amw_assert(window);
    return window->flags;
}
...
void amw_window_should_close(amw_window_t *window, bool value)
{
    if (!hadal.initialized)
        return;
    amw_assert(window);
...
void amw_window_visible(amw_window_t *window, bool value)
{
    if (!hadal.initialized)
        return;
    amw_assert(window);
...
void amw_window_minimize(amw_window_t *window)
{
    if (!hadal.initialized)
        return;
    amw_assert(window);
...
void amw_window_maximize(amw_window_t *window)
{
    if (!hadal.initialized)
        return;
    amw_assert(window);
...
void amw_window_restore(amw_window_t *window)
{
    if (!hadal.initialized)
        return;
    amw_assert(window);
...
void amw_window_fullscreen(amw_window_t *window, bool value)
{
...
void amw_window_retitle(amw_window_t *window, const char *title)
{
    if (!hadal.initialized)
        return;
    amw_assert(window);
...
void amw_window_resize(amw_window_t *window, int32_t width, int32_t height)
{
    if (!hadal.initialized)
        return;
    amw_assert(window);
...
void amw_window_size(amw_window_t *window, int32_t *width, int32_t *height)
{
    if (width)
        *width = 0;
    if (height)
...
void amw_window_framebuffer_size(amw_window_t *window, int32_t *width, int32_t *height)
{
    if (width)
        *width = 0;
    if (height)
...
void amw_window_content_scale(amw_window_t *window, float *xscale, float *yscale)
{
    if (xscale)
        *xscale = 0;
    if (yscale)
