...
#if defined(AMW_PLATFORM_EMSCRIPTEN)
    #ifdef SEWING_STACK_GUARD
        #undef AMW_SEWING_STACK_GUARD
    #endif
#endif
...
amw_static_assert(sew_context_size_assumption, sizeof(intptr_t) == sizeof(amw_sewing_t *));
amw_static_assert(size_t_can_hold_pointers, sizeof(size_t) >= sizeof(void *));
                  ...
                  amw_procedure_argument_t main_procedure_argument)
{
    ...
    for (size_t i = 0; i < sewing->fiber_count; i++) {
        atomic_store_explicit(&sewing->free[i], i, memory_order_release);
