...
#ifdef AMW_PLATFORM_EMSCRIPTEN
    #if (defined(__STDC_VERSION__) && __STDC_VERSION__ >= 201112L)
        #include <emscripten/atomic.h>
        #define AMW_ATOMIC_USE_EMSCRIPTEN_C11 1
    #else
        #define AMW_ATOMIC_USE_EMSCRIPTEN_C11 0
    #endif
#endif
...
bool amw_spinlock_try_acquire(uint32_t *lock)
{
    ...
    #if AMW_ATOMIC_USE_EMSCRIPTEN_C11
    return emscripten_atomic_exchange_u32(lock, 1);
    #else
    return __sync_bool_compare_and_swap(lock, 0, 1);
    #endif
...
void amw_spinlock_acquire(uint32_t *lock)
{
...
void amw_spinlock_release(uint32_t *lock)
{
#if defined(_MSC_VER) && (defined(_M_ARM) || defined(_M_ARM64))
...
#elif defined(AMW_PLATFORM_EMSCRIPTEN)
    #if AMW_ATOMIC_USE_EMSCRIPTEN_C11
    emscripten_atomic_exchange_u32(lock, 0);
    #else
    __sync_bool_compare_and_swap(lock, 1, 0);
    #endif
#elif defined(__WATCOMC__) && defined(__386__)
    amw_compiler_barrier();
    *lock = 0;
#elif defined(__GNUC__) && defined(__GCC_HAVE_SYNC_COMPARE_AND_SWAP_1)
    __sync_bool_compare_and_swap(lock, 1, 0);
#else
