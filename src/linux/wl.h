...
typedef struct amw_wayland_cursor amw_wayland_cursor_t;
#define AMW_HADAL_CURSOR_WAYLAND_STATE amw_wayland_cursor_t wl;

typedef struct amw_wayland_window amw_wayland_window_t;
#define AMW_HADAL_WINDOW_WAYLAND_STATE amw_wayland_window_t wl;

typedef struct hadopelagic_wayland hadopelagic_wayland_t;
#define AMW_HADAL_HADOPELAGIC_WAYLAND_STATE hadopelagic_wayland_t wl;
...
extern void             AMW_CALL hadal_wayland_free_output(amw_output_t *output);
extern amw_vidmode_t *  AMW_CALL hadal_wayland_video_modes(amw_output_t *output, int32_t *count);
extern bool             AMW_CALL hadal_wayland_video_mode(amw_output_t *output, amw_vidmode_t *mode);

extern void             AMW_CALL hadal_wayland_get_output_position(amw_output_t *output, int32_t *xpos, int32_t *ypos);
extern void             AMW_CALL hadal_wayland_get_output_content_scale(amw_output_t *output, float *xscale, float *yscale);
extern void             AMW_CALL hadal_wayland_get_output_workarea(amw_output_t *output, int32_t *xpos, int32_t *ypos, uint32_t *width, uint32_t *height);
...
extern void             AMW_CALL hadal_wayland_resize_window(amw_window_t *window, uint32_t width, uint32_t height);
extern void             AMW_CALL hadal_wayland_window_size(amw_window_t *window, uint32_t *width, uint32_t *height);
extern void             AMW_CALL hadal_wayland_framebuffer_size(amw_window_t *window, uint32_t *width, uint32_t *height);
...
extern void             AMW_CALL hadal_wayland_update_buffer_scale_from_outputs(struct amw_window *window);

#else
    /* null the wayland state */
    #define AMW_HADAL_OUTPUT_WAYLAND_STATE
    #define AMW_HADAL_CURSOR_WAYLAND_STATE
    #define AMW_HADAL_JOYSTICK_WAYLAND_STATE
    #define AMW_HADAL_WINDOW_WAYLAND_STATE
    #define AMW_HADAL_HADOPELAGIC_WAYLAND_STATE
